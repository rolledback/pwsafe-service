...
func (p *Provider) GetAuthURL(ctx context.Context, sessionID string) (string, error) {
	if p.AuthError != nil {
		return "", p.AuthError
	}
...
func (p *Provider) HandleCallback(ctx context.Context, code string, state string, sessionID string) error {
	if p.AuthError != nil {
		return p.AuthError
	}
